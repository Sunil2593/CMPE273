        ...
        if(!(username==null) && !(username==""))
        {
            if(!(password==null) && !(password==""))
            {
                ...
                con.connect(function(err) {
                    if (err)
                        throw err;
                    //console.log("Connected! Name: "+name+ " Username: "+username+ " Password: "+password );
                    var sql = "INSERT INTO users VALUES ?";
                    var records =[
                        [0,null, name, username, null, null, 'JAVA', password]
                    ];
                    con.query(sql, [records], function (err, result, fields) {
                        if (err)
                            throw err;
                        answer="Your Signup is Successfull";
                        res.json({"answer":answer});
                    });
                });
            }
